...
function loadEndpointConfig(
  ...
  const environment = (getEnv("ENVIRONMENT") as "development" | "staging" | "production") || "production";
  ...
  return {
    ...
    credentials: {
      ...
      clientSecret: getEnv("CLIENT_SECRET") || defaults.credentials?.clientSecret,
      customHeaders: defaults.credentials?.customHeaders,
    },
    timeout: parseInt(getEnv("TIMEOUT") || String(defaults.timeout || 30000), 10),
    maxRetries: parseInt(getEnv("MAX_RETRIES") || String(defaults.maxRetries || 3), 10),
    retryDelay: parseInt(getEnv("RETRY_DELAY") || String(defaults.retryDelay || 1000), 10),
...
export function getRealScoutConfig(): MCPEndpointConfig {
  return loadEndpointConfig("REALSCOUT", {
    ...
    providerConfig: {
      ...
      widgetScriptUrl: "https://em.realscout.com/widgets/realscout-web-components.umd.js",
    },
  });
}
