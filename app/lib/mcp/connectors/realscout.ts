...
export class RealScoutConnector extends HTTPMCPClient {
  ...
  private getAgentEncodedId(): string {
    const config = this.config?.providerConfig as { agentEncodedId?: string } | undefined;
    return config?.agentEncodedId || process.env.MCP_REALSCOUT_AGENT_ID || "QWdlbnQtMjI1MDUw";
  }
  ...
  getWidgetScriptUrl(): string {
    const config = this.config?.providerConfig as { widgetScriptUrl?: string } | undefined;
    return config?.widgetScriptUrl || "https://em.realscout.com/widgets/realscout-web-components.umd.js";
  }

  /**
   * Build widget attributes for embedding
   */
  buildWidgetAttributes(options: RealScoutListingsOptions = {}): Record<string, string> {
   ...
   */
  async getListings(options: RealScoutListingsOptions = {}): Promise<MCPResponse<RealScoutListing[]>> {
