...
export function getRealScoutAgentId(): string {
  ...
  const agentId = (config.providerConfig as { agentEncodedId?: string } | undefined)?.agentEncodedId;
...
export function getRealScoutWidgetScriptUrl(): string {
  const config = getRealScoutConfig();
  const scriptUrl = (config.providerConfig as { widgetScriptUrl?: string } | undefined)?.widgetScriptUrl;
  return scriptUrl || "https://em.realscout.com/widgets/realscout-web-components.umd.js";
}
